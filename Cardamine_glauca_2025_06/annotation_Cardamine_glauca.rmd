---
title: "Annotation - Cardamine glauca"
...
- Change folder for the species (`Cardamine_glauca_2025_06`) throughout the document.
- Change genome_assembly variable (`genome_assembly="Cardamine_glauca_CUNI_V1_2024_10_masked.fa"`) throughout the document.
...
- Change species in Braker run (`--species=Cardamine_glauca`).
...
In the previous version of this script, I used assembly named `$genome_assembly`. This was now renamed to `Cardamine_glauca_CUNI_V1_2024_10_masked.fa`, but it is the same assembly.
...
Macrogen has higher Quality Scores, but the newer Novogene data are similar. On the other hand, they have ~ 15 % of reads with adapters, some overrepresented sequences and some small bumps in "Per Sequence GC Content".
...
```
For this run, we have 45030 transcript. The number of transcripts in previous run (without flowers and flower buds RNAseq) was 39064, so the increase is significant. The flowers and flower buds were sequenced deeper compared to other samples (60M reads vs. 30M reads).

Similarly, the number of genes from StringTie (alternative transcripts in one locus are assigned to one gene) increased from 27054 to 31004.
...
#### Comments to results
- All Busco results are very similar to Noccaea, even that Cardamine has much less annotated genes.
- Including RNAseq from flowers and flower buds increased the Busco scores from 97.5% to 98.1%.
...
I will compare genes predicted by Braker for the current run and the previous run without RNA from flowers and flower buds.
...
The current run of Braker predicted 24828 genes, the previous one without RNA from flowers and flower buds predicted 24376 genes. Of these, 23480 have exactly the same coordinates.
...
- udělat tabulku "gene reliability"
...
# basic statistics
agat_sq_stat_basic.pl -i ../results_braker_01/braker.gtf -g ../genome_assembly/$genome_assembly > stat_basic_braker.gtf.txt

# detailed statistics
agat_sp_statistics.pl -i ../results_braker_01/braker.gtf -g ../genome_assembly/$genome_assembly > stat_detailed_braker.gtf.txt
...
# run Busco
busco --in $genome_assembly --mode genome --lineage_dataset brassicales_odb10 --cpu 2
...
```{r}
setwd("D:/!ecolgen/annotations/Alyssum/Cardamine_glauca_2025_06")
